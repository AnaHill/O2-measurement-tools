...
% 22-08-2108
   ...
   % No more requires SV_fit.m (function inside --> needs MATLAB 2016b or newer)
% 14-03-2018
...
% x=lsqcurvefit(@SV_fit,sqrt([22,.2]),phi,po2);
% xphi0est=lsqcurvefit(@phi0est,sqrt([22,.2]),phi,po2);
% xphi0est=xphi0est.^2;

% lsqcurvefit: solve problem 
% min  sum {(FUN(X,XDATA)-YDATA).^2}  where X, XDATA, YDATA and the  X
% values returned by FUN can be vectors or matrices
...
% % % end
% % % 
% % % function phi0est=phi0estimation(x,xdata)
% % %    xc=x(1).^2;
% % %    ksv=x(2).^2;
% % %    xdata=xdata-xc;
% % %    phi0est=xdata*(1+ksv)-xc;
% % % end
