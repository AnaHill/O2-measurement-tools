...
if iseResponseInfo % step up
   ...
   DatTempo = mave(DataO2filtered ,5);
   Delay_time = timeVector(find(DatTempo >= y_delay ,1));
...
% Method 1) time constant: DelayLevel% to ~63.2% (= 1-1/exp(1))
   % time to reach 63.2% reduced by delay time
...
Dat_= Data_O2(delay_point:end,2);%DataO2filtered(delay_point:end);
tim_=timeVector(delay_point:end);
tim_=tim_-tim_(1);
% intial values for 1st order fit: 
A = TotRange; % = abs(Values1;Values2)
tau_ = 100;%? initial guess
...
if iseResponseInfo % step up: y = A*exp^(-t/tau) + D
   ...
   %fit_plot2=plot(tim_,Y(1)*exp(-tim_/Y(2))+Y(3),'--g');
