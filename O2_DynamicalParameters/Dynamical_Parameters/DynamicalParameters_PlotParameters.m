...
%%%%%%%%%
Data_To_Analyzed = DataChosen; 
Data_To_Analyzed(:,1) = Data_To_Analyzed(:,1)-Data_To_Analyzed(1,1);
legs={}; timeVector = Data_To_Analyzed(:,1); close;
hparam=figure('Name','Calculating parameters','units',...
   'normalized','outerposition',[0 0 1 1]);
hmeas = plot(timeVector,Data_To_Analyzed(:,2),'o','Markersize',4,...
...
DatafilteredORG=Datafiltered; timeVectorORG = timeVector;
timeVector = [0:max(timeVector)]';
Datafiltered = [interp1(timeVectorORG,DatafilteredORG,timeVector,'pchip')]; 
hmeasfilt = plot(timeVector,Datafiltered,'linewidth',3);%,'*')
...
switch Method_to_take_max_and_min_values
   case 1 % median
      Values1 = median(Datafiltered(1:Nsamples));
      Values2 = median(Datafiltered(end-Nsamples+1:end));
   case 2 % mean
      Values1 = mean(Datafiltered(1:Nsamples));
      Values2 = mean(Datafiltered(end-Nsamples+1:end));
end
...
while ~strcmp(chosen_Values ,'Yes') 
   ...
   if strcmp(chosen_Values ,'No, choose with a mouse') 
      ...
      Values1=median(Datafiltered(start_point:end_point)); 
      if exist('hlin','var')
      ...
      start_point = find((timeVector(:,1)) >= x_val(1),1);
      end_point = find((timeVector(:,1)) >= x_val(2),1);
      Values2=median(Datafiltered(start_point:end_point));
      if exist('hlin','var')
...
if iseResponseInfo % step up
   ...
   t10 = timeVector(find(Datafiltered >= y10,1));
   t90 = timeVector(find(Datafiltered >= y90,1));
   %%%%%%% delay: time to reach Delaylevel amount of change   
   y_delay = DelayLevel*TotRange + min([Values1,Values2]);
   Delay_time = timeVector(find(Datafiltered >= y_delay ,1));
   delay_point = find(timeVector >=  Delay_time,1);
   yStep = (1-1/exp(1))*TotRange + min([Values1,Values2]);
   tStep = timeVector(find(Datafiltered >= yStep,1));
else % step down
   TotRange = Values1 - Values2;
   y10 = 0.1*TotRange + min([Values1,Values2]);
   y90 = 0.9*TotRange + min([Values1,Values2]);
   t10 = timeVector(find(Datafiltered >= y10));t10=t10(end);
   t90 = timeVector(find(Datafiltered >= y90));t90=t90(end);
   %%%%%%% delay: time to reach Delaylevel amount of change
...
Dat_= Datafiltered(delay_point:end);
...
%   [Ytest,Val] = fminsearch(@(X)sum( (Dat_-(X(1)*exp(-tim_/X(2))+X(3))).^2), X0,options_sol );
...
StepInformation = stepinfo(Datafiltered,timeVector,...
...
if iseResponseInfo % step up
   htit=title([name_file(1:end-4),10,'Step up: Rise time t(10%-90%): ',...
   ...
   htit=title([name_file(1:end-4),10,'Step down: Fall time t(90%-10%): ',...
      num2str([Rise_time],4),' s',10,...
      'Two time constants t(99.9%-63.2%) & tau from the fitted plot: ',...
      num2str([Time_Constant],4),' & ',num2str([Time_Constant2],4),' s',10,...
