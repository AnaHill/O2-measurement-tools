%% Reading O2 sensor and plotting & saving (read more info in the end)
   % Requires files: 
      ...
      % For changing calibration values during the measurement: 
         % O2_ChangeSensorCalib_parameters.txt
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Version 1.35 updates
   % Can be stopped during pause (ugly while-loop method used)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
clear, close all
...
pause_time = str2num(answer{:}); % sets pause time (in sec) before next measurement

answer_testing = questdlg(['Measurement or testing script?'], ...
   'Measurement or Testing','Measurement','Testing', 'Measurement');
if strcmp(answer_testing ,'Testing')
...
while(ishandle(H))
      if exist('subplotting','var') && subplotting == 1 
         RunO2sensor_ver_1_35_subplotting
      end
   % Reading data
   if exist('Testing_Script','var') && Testing_Script == 1  % for testing purposes
   ...
   if exist('pause_time','var') && pause_time > 0.5 % if pause before next measurement
      pause_round = round(pause_time);
      x_pause = 1;
      while x_pause <= pause_round
         if (ishandle(H))
            pause(1)
            x_pause = x_pause+1;
            if exist('subplotting','var') && subplotting == 1 
...
%%%% Calibration parameters 1) KSV, 2) phi0, 3) caldevi
   ...
   % Changing during measurement: New values read from_ O2_ChangeSensorCalib_parameters.txt
...
%%%% Without plotting during the measurement: set -> FigsToPlot=[];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Future improvements %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Bettr method to stop during pause state 
