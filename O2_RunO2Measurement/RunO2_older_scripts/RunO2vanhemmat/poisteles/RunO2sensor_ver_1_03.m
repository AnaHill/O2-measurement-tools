...
% Version 1.03 updates
   % addpoints replaced by line commmand
   % bug fixs: 
      % amp miss calculated by: mean(abs(Measurement-StopCal)/180*pi);
         % --> now corrected amp=mean(abs(Measurement-StopCal));
   % information moved to the end
clear, close all
   ...
   while(ishandle(H))
      for pp = 1:length(FigsToPlot)
         ...
         switch FigsToPlot(pp)
            ...
            case 2
               ylabel('Amplitude ()'); 
               %h2 = animatedline('Marker','v','Color','g'); 
            case 3
               ylabel('O_2 (%)'); 
