...
clear, close all
pause_time=0.1; % sets pause time (in sec) before next measurement
FigsToPlot=[1:3];  % 1 = Phase, 2 = Amp, 3 = pO_2
...
for pp = 1:length(FigsToPlot)
      ...
      switch FigsToPlot(pp)
         case 1               
            ylabel('Phase (deg)');
            % vanha tyyli: luotiin animatedline jokaisessa figuressa
            % h1 = animatedline('Marker','o','Color','r'); 
         case 2
            ylabel('Amplitude ()'); % h2 = animatedline (...
         case 3
            ylabel('O_2 (%)'); % h3 = animatedline (...
      end
      xlabel('time [s]')
end
x=1;
while(ishandle(H))
   ...
   t_(end+1)=tnow;  
    ...
    % drawing
      % vanha tyyli addpoints komennolla: addpoints(h3,tnow,pO2);drawnow;
