...
% Version 1.1 updates
   % For plotting: addpoints used instead of line command (better memory usage)
   % Now during while loop, variables length are only 1 
      % During loop, saving new line to .txt file 
   % will delete one_meas.mat poistaa kierroksen jälkeen one_meas.mat tiedoston, ja kun dos komento ->
      % lukee kunnes löytyy
clear, close all
pause_time=0; % sets pause time (in sec) before next measurement
...
if exist('FigsToPlot','var') && length(FigsToPlot) >= 1
   ...
   while(ishandle(H))
      ...
      if exist('Testing_Script','var') && Testing_Script == 1  % for testing purposes
         ...
         y=dos('pdamp.exe -p7 -t30 -i40 -mp -f2000 -d50 -cz one_meas.mat');
         while~(exist('one_meas.mat','file'))
            % do nothing, check as long as file one_meas.mat exists
         end
         load one_meas.mat
         % deleting first 10 samples (device might be slightly warming etc.)
         if length(Measurement) >= 20
...
% c) For plotting during measurement, would comet/else be better than addpoints?
...
% e) Write a log file after each step to see if/when measurement stopped
%%%% NEW after discussion with Pekka Kumpulainen
% f) Piirtäisi aina gui-ikkunan, jossa näppäimet: Plot, stop, calibration changes
   % pystyisi muuttamaan kalibrointiarvoja
