...
% Requires files: 
   ...
   % PDAMP.exe --> measurement file
% Notice: Following fileas are no more needed
   % any text file (.txt) related to calibration
   % SternVolmerC.m 
...
annotation('textbox',[dim(1)-0.05 dim(2:end)],'String','Ksv',...
...
annotation('textbox',[dim2(1)-0.05 dim2(2:end)],'String','phi0',...
   'Fontsize',14,'Fontweight','bold','interpreter','none',...
   'Linestyle','none','FitBoxToText','on');
...
annotation('textbox',[dim3(1)-0.07 dim3(2:end)],'String','caldevi',...
   'Fontsize',14,'Fontweight','bold','interpreter','none',...
   'Linestyle','none','FitBoxToText','on');
dimPause = [.5 .3 .08 .1]; 
...
annotation('textbox',dimPauseText,'String','Pause time between measurements (in sec) ',...
   'Fontsize',14,'Fontweight','bold','interpreter','none',...
   'Linestyle','none','FitBoxToText','on');
...
while(ishandle(H)) % Continuing while Stop button is not pressed
   ...
   if exist('change_calibration','var') && change_calibration == 1
      O2RunMeas_ChangingCalibrationAndPausing
   end
   %%%%%%%%%%%%%%%%%%
   % Converting to pO2 [%]
